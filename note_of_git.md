...
基本流程:     
在文件夹创建仓库 --> 在工作区编写文件，包括文件的增删改 --> 把修改提交到暂存区 --> 把暂存区的修改提交到代码库 --> 提交到云端
...
这个.git文件夹保存了git的相关配置信息和版本，有了它就可以进行版本的管理了   

## 创建版本

1. 新建一个文本文件test.txt --> 在已被追踪的文件夹中新增了一个文件
2. git add test.txt  --> 把这个文件加入到暂存区
3. git commit -m 'version0.1'  --> 起一个有意义的内容，并用commit提交到代码库
4. git log --> 查看版本记录 --> 可以发现commit后有版本信息
5. 修改test.txt内容    --> 这个文件被修改了，再次产生一个版本
6. git add test.txt   --> 提交到暂存区
7. git commit -m 'version0.2' --> 再次提交到代码库，此时这个文件夹有两个有两个不同的版本，一个是新增的操作，一个是修改文件的操作

git记录的是修改的内容，后面的版本依赖于前面的版本
![描述1](image/1.png)

从上面的操作，可以发现任何在工作区的操作(增、改、删)都会被git自动追踪   
这些改动可以通过`git status`来查看  
...
是将已经提交到代码库的记录回退，即已经commit之后的代码  
...
这时，可以使用版本号来回退，版本号就是git log后出现的commit后面的内容   
版本号很长，用前几个即可  
`git reset --hard [版本号]`
...
可以发现，`git log`只显示当前HEAD位置之前的版本号，那么**当HEAD在版本1时，如何查看其他版本的版本号(commit号)?**      
...
在开发过程中，通常是一边运营一边修改，在项目上线之后，主分支(master/main)分支运行的是稳定版本，不会轻易地去修改。    
这时要新增或删除功能怎么办？  
...
在连接远程服务器之前，需要配置git，让远程服务器知道你是谁  
