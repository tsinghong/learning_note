...
程序本质上就是由指令与数据组成的，指令用来告诉计算机应该做什么，数据就是程序操作的对象  
...
在官网上下载解释器，要注意下载与你操作系统相匹配的解释器，比如我的操作系统是windows 64位，就下载windows installer(64bit)  
下面我们介绍一种比较方便实用的开源版本解释器anaconda   

## anaconda--开源解释器

它主要是针对个人用户免费，企业版本还是需要收费的，毕竟企业是用它来赚钱的嘛，收点还是应该的，毕竟开发也是需要成本的。

### 关于anaconda

什么是anaconda?          
它是一款开源的python发行版本，包含python的解释器和环境管理及一些包，所以下载了anaconda安装好以后，就有了解释器和一些基本的工具，可以用它来管理python包，创建虚拟环境等等。   
...
与时俱进是一个好习惯，你应当学习新版python所支持的一些新特性，所以，你的电脑里会有不同的python版本。  
但是你的电脑里有一些已经安装好的包，你需要能够支持它的解释器去调用。例如，某个包你在python3.5版本下安装了，在python3.7版本也安装了，但是这个包的版本不一样，这就非常容易导致混乱，这时候你就需要虚拟环境来将每个解释器和包“包装”起来，防止放乱了。   
这个包装我们就称之为虚拟环境。   
python3.5以后自带了一个虚拟环境的管理器venv，同样的，自带的通常都没那么好用    
...
**什么是环境变量？**   
简单的说，就是在命令行工具中优先查找的位置，  
系统会优先从环境变量中查找你所输入的命令。   

**关于conda的环境变量**          
windows下要把anaconda文件夹下的scripts文件夹加入到系统的环境变量中，  
然后可以通过cmd或者powershell的conda --version命令来查看是否添加成功    
...
**下面运行一个.py文件：**   
首先进入这个文件夹C:\Users\Administrator，新建一个txt文档，并把名称改为1.py，注意要把原来的.txt删除     
![first_py_file](image/first_py_file.png)   
打开这个文档并输入`print("hello world")`  
![first_py_code](image/first_py_code.png)   
然后打开cmd，输入`python 1.py`就可以看到运行结果了
![first_run_py](image/first_run_py.png)  


### IDE--集成开发环境

刚才我们在一个文本文件中写了代码，但是现代我们拥有了更加强大的工具，可以帮助我们更快速地编写代码。
现代的IDE通常都集成了强大的自动补全、语法检查、调式等功能，常用的IDE有Pycharm、Visual Studio等等，它们都包含了一些开箱即用的功能，就是安装好就可以用了，当然，它们也都是收费软件。   
这里我们选用了微软的Vscode这一款开源编辑器，它需要一些配置，也需要单独地去学习怎么用，这里，我们只简单地介绍一下如何使用它，更多的配置和快捷键等请参看另外一篇Vscode的教程。    
首先，请到官网下载[下载地址](https://code.visualstudio.com/)    
由于之前已经安装好了python的解释器，它会自动检测到解释器的位置，并且建议你安装python扩展，安装之后就可以在工作区编写代码了   
...
print()的功能就是输出到终端     
至于为什么总是要带着括号及函数的其它知识，我们将在函数一节详细讲解   

## 注释

**什么是注释？**  
注释就是用来解释代码的文字，但是在实际运行过程中，解释器会忽略掉注释内容不执行  

**注释的功能**
1. 解释一行或一段代码
2. 方便他人查看、了解代码
3. 忽略掉某行或某段代码，然后进行调试方便找出BUG或更新功能
...
**Python是一种弱类型的语言，在使用变量的时候不需要事先规定好变量的种类。**  
...
也就是说，python解释器在运行到`name = 'xxx'`和`l = ['a', name]`时向操作系统申请内存空间，这一过程很快。   
...
[pep8的原文](https://www.python.org/dev/peps/pep-0008/)   
...
以下作为了解，以后会用到，记得回来看。目前了解即可     

**Package and Module Names 包名和模块名**  
模块应该用简短全小写的名字，如果为了提升可读性，下划线也是可以用的。Python包名也应该使用简短全小写的名字，但不建议用下划线。
当使用C或者C++编写了一个依赖于提供高级（面向对象）接口的Python模块的扩展模块，这个C/C++模块需要一个下划线前缀（例如：_socket）
...
2. float型：浮点数，如1.0、2.2、3.3等等，小数点后边的位数称为精度，位数越多精度越高
   + 形式：`a = 1.0`；`b = 3.1415926`

3. str型：字符串型，如"abc"、"123"等等，字符串类型都需要单/双引号括起来，即使是数字在引号内也是字符串型
   + 形式：`a = 'abc'` 注意：`c = '123'`这里是字符串类型的数字，而不是整型

4. 列表(list)：一串数据的合集，有序，计数从0开始而不是1；可列表嵌套使用
   + 形式：`a = ['abc', 2, "cde"]`；嵌套：`a = [1, 2, ['a', 'b']]`

5. 字典(dict)：以key-value(键-值)的形式存储数据，key表示的是值的位置，且key值必须为不可变类型，即不可使用容器类型
   + 形式：`a = {a:1, b:2, c:"abc"}`
...
==> xxx
```
可以看到t[1]的值是'xxx'，因为值'xxx'在内存中的地址没有变，给name重新赋值，只是改变了name这个变量与'xxx'的绑定关系  
t[1]依然指向值'xxx'的内存地址，与变量名无关        
![图示](image/列表地址1.png)  
![图示](image/列表地址2.png)  
字典：key对应内存地址
...
什么是方法(method)？  
方法本质上是函数，代表一种功能  
方法一词来源于面向对象，在 python 中基本类型也是一种对象  
函数在对象中，就称为方法，这一点在面向对象一章中会详解，这里了解即可  

内置方法就是 python 帮你写好的方法，可以直接使用而不需要再去单独写这项功能  
基本类型的方法：python 针对基本类型，内置的一些功能，可以更方便的达到一些操作数据的目的     
...
str()函数可以把任意其他类型都转成字符串，字符串类型类似于列表，可以按索引取值     
`msg='hello world'`      
正向取      
`print(msg[0])`   
`print(msg[5])`  
...
判断的结果是布尔型  
    ...
    print("lilei" in "lilei is handsome")       
    print("lilei" not in "lilei is handsome")
    print(not "lilei" in "lilei is handsome") # 不推荐使用
...
方法二：join方法 -->  在*每个字符*后添加一个字符    
把列表拼接成字符串，或在字符串中添加字符，该方法的参数为字符串或列表    
`'某字符'.join(参数)`具体是把某字符添加到参数的每个字符后面   
比如参数有100个字符，则这99个字符(除最后一个字符)的每一个后面都会加上某字符  
...
isdigit:判断字符串是否由纯数字组成
    ...
    print('123'.isdigit())
    print('12.3'.isdigit())   # ==> False 说明只判断整型
...
方法很多，了解即可，遇到具体情况时知道有工具可以这么用，回过头来看一看，不建议死记硬背  
...
# range()函数，根据参数生成连续数字生成器，通过list()函数转换成列表
# 例如range(10)，生成0到9的数字生成器，range(1, 50)生成一个0到49的生成器
# 遵循左闭右开原则，即取不到右边的数字  
# 关于生成器在函数一节详解  
print(res[1])
===> 1
...
4. sort方法：排序，要求列表内的元素必须相同，原列表不会被该表，默认升序，可以传入参数（reverse=True）--> 降序

### 元组类型

同列表一样是有序的，但是只能读取不能改，任何对元组内元素修改的操作均会报错         
元组中也存放的是内存地址    
...
注意：字典也是无序的，所以只能根据key来取出value，而不可通过索引     
...
**删**   
1. 通用删del
2. pop方法：传入key作为参数，返回被删除key的value值，对比列表的pop()，因为字典无序，所以是随机删除  
...
c = b * 10
print(f'{a}的10倍是', c) 
# 其中f'{a}'是一种格式化输出的写法，下面会详细讲到
# 在这里记住即可，{a}花括号的内容会被c所替代，然后被打印出来

# 以上代码可以简写
a = int(input("请输入一个数字，你将得到它的10倍："))
print(f'{a}的10倍是', a*10)
```
input()函数内的参数是一个字符串，它会被打印出来。通常，这个字符串写一些提示信息，告诉用户要输入什么内容   
...
可以看到，输出的形式根据传入的参数不同发生了变化，这些参数都是可以根据实际需要来替换成其他字符  

参数file指定了print()函数的输出位置，这里默认的是`file=sys.stdout`即系统标准输出，它告诉print()函数   
把内容输出到屏幕上。当然，也可以输出到其他地方，比如一个文件。

    f = open('a.txt', mode='wt', encoding='utf-8')
    print("春眠不觉晓", file=f)
    f.close()

我们可以看到在.py被执行的文件夹内多一个a.txt的文件，print()的内容被写入   
至于`f = open()`是一个打开文件的操作，以后会详解  

参数flush是刷新缓存的参数，一直用默认的False就好    
...
%的形式因其底层实现的关系，效率低，并不推荐

#### 2. str.format:兼容性好

这种方式起源于python2.6时代，有三种用法  
按照位置传值
...
按照索引传值  
    ...
    print(res)
    # 所有{0}的位置均被填充了Lilei，{1}的位置被填充了18
    # 在.format('Lilei', 18)中，'Lilei'所在位置的索引是0
...
这种方法是在一个字符串的引号前加上字母f，就可以在字符串内的花括号直接使用变量   
python3.5以后才推出，效率高，推荐
...
这里主要用到字符串的`.format()`方法  
先取到值,然后在冒号后设定填充格式：`[位置参数]:[填充字符][对齐方式][宽度]`

    # *<10：左对齐，总共10个字符，不够的用*号填充
    # {}中的0会被.format()中的第一个参数所替代，别忘了
...
有时，需要输出浮点型数据，但是这个数的小数点后面很长，不需要全部都输出，就需要对输出的精度进行控制  
...
### 补充：一些常用的转义字符
    ...
    # \n表示换行，在print()函数中默认的结尾
    print("多了一个空行！\n")

    # \t表示tab，即制表符，很特殊，可以单独表示一个tab键，也可以定义成多个空格
    # 通常，它默认是由四个空格组成
    print("中间\t被隔开")
...
比较运算符用于比较两个变量的值，包括以下几种  
大于，大于等于，小于，小于等于，等于（判断），不等于       

    >、>=、<、<=、==、!=、is
    结果为布尔值

`is`也叫身份运算符，也是用于两个变量的比较  
对比： is 与 == 及 != 运算符的不同，后两者比较的是两个变量的值，而 is 是比较两个变量的ID  
根据之前深浅copy我们知道，同一个值的变量不一定是同一个地址   
同一个id的变量值一定是相同的  

逻辑运算符则用于逻辑判断，包括与、或、非三种

    not、and、or

and 和 or 用于连接多个表达式来进行判断  

    print(1 > 2 and 2 > 1 and 3 > 1)  # False ,只要有一个结果是假，整个表达式就为假
    print(1 > 2 or 2 > 1 or 3 > 1)    # True ,只要有一个结果是真，结果就为真
    print(1 > 2 and 2 > 1 or 3 > 1)   # True ,先计算 and 的结果 --> 假，再计算 or 的结果 --> 真

not 是取反操作，把计算结果直接取反，即表达式一为真，则 not 表达式一为假

    print(not 1 > 2)  # True ,表达式 1 > 2 为假，加上 not 则表示相反的结果

三种逻辑运算符的优先级：not > and > or  

同样用于判断的成员运算符: in 和 not in
...
什么是流程？  
流程就是做一件事的先后顺序，流程控制包括了if分支控制和循环控制   

流程控制就像是一个工厂的生产线
1. 首先要判断是否有原料  
2. 没有原料就要去购买，然后才能生产  
3. 有原料的话就进入下一步开始生产  
4. 一个工厂不可能只生产一件产品，重复上述过程，产生循环   
...
if在英语中的意思是如果，在逻辑学中，“如果”开头的语句代表假言判断   
如果 + 条件，条件成立，就执行if语句块中的语句，不成立则跳过   
这里提到了语句块，那么这么多的代码Python是如何区分哪些是if的语句块呢？  
这里就用到了Python的语法规范问题。在Python中，把缩进的语句视为最近没有缩进语句的子语句  
```
...
下面是if的基本语法，尤其要注意，判断条件后面要加上冒号`:`，这个表示条件判断的结束，忘了就会报语法错误    
...
**if 的特殊用法**  
if...else... 还可以作为一个三目运算符，用一句表达式即可得出结果  

    a = 6
    b = 8
    print("a比b大" if a > b else "a比b小")

如果 if 判断结果为真，则运行 if 前面的表达式，反之，则运行 else 后的表达式

## 循环--while和for

循环，顾名思义，就是重复做一件事，这可是计算机非常擅长的，解放了大量的人力   
...
while 条件1：
    ...
    代码4
```
ps：break 只会结束循环，不会结束if判断  
条件2为真，执行代码3和 break，while 被结束了  
else的子代码`代码4`就不会被运行   
暂时没想到好例子...
当条件2位假，if 被跳过，while 循环正常结束之后  
就会运行代码4

**for循环**   
理论上for循环能做的事情，while循环都可以做，但是 for 循环在循环取值（遍历取值）比while循环更简洁   
...
循环内可以嵌套 if 语句，同样也可以嵌套循环  



# 字符编码简介

因为下面要学到文件操作，文件操作的基础是文本文件，文本文件则涉及到计算机如何去认识这些字，所有需要对字符编码有一定的了解  
...
编码表由操作系统提供，启动后加载到内存，所以同一个文件在不同系统会出问题，除非用统一的编码表    
  
windows系统 cmd 中文表示用的是 gbk 表示   
大多数编程语言用到的是 utf-8 编码    
...
GBK表：
1. 支持英文字符、中文字符
2. 采用8位（8bit=1Bytes）二进制数对应一个英文字符串，同 ASCII    
...
理论上是可以将内存中 unicode 格式的二进制直接存放于硬盘中的，但由于 unicode 固定使用两个字节来存储一个字符，  
如果多国字符中包含大量的英文字符时，使用 unicode 格式存放会额外占用一倍空间（英文字符其实只需要用一个字节存放即可）       
然而空间占用并不是最致命的问题，最致命地是当我们由内存写入硬盘时会额外耗费一倍的时间    

utf-8：       
将内存中的 unicode 二进制写入硬盘或者基于网络传输时必须将其转换成一种精简的格式，   
这种格式即utf-8（全称Unicode Transformation Format，即 unicode 的转换格式）  
+ 英文 -> 1Bytes(8bit)
+ 汉字 -> 3Bytes(24bit)

结论：
1. 内存固定使用 unicode，我们可以改变的是存入硬盘采用格式，unicode 更像是一种过渡版本，用来兼容其他诸如 gbk, shift-jis 等
+ 英文 + 汉字 -> unicode -> gbk
+ 英文 + 日文 -> unicode -> shift-jis
+ 万国字符 -> unicode -> utf-8
  + gbk\shift-jis\utf-8 是平级的
...
1. python 解释器默认读文件的编码
+ python3 默认：utf-8
+ python2 默认：ASCII

2. 保证运行 python 程序前两个阶段不乱码的核心法则：指定文件头，修改解释器的编码格式
...
3. python3 的 str 类型默认直接存成unicode格式，无论如何都不会乱码
...
在 Python 中，对文件的操作主要有三种：r 模式(读)，w 模式(写)，a 模式(追加)   
简单地分析一下这三种模式：  
1. r 模式：从文件中读出数据，只是读，不做任何其他操作
2. w 模式：将某些内容写入文件，只是写，不会读，也不关心文件里面是否有内容
3. a 模式：将某些内容写入文件的后面，如果原文件有内容，就在原内容的后面写，也不会读

额外模式：t 模式和 b 模式      
**注意：t 和 b 不能单独使用，必须跟 r/w/a 连用**    
基本形式就是：`mode=rt`，`mode=wb`，`mode=at`等等   
可以任意组合，不局限于上面的三个例子   

t 模式：文本模式（默认的模式），例如`mode=r`，r后面就会有个省略的 t    
1. 读写都以str（unicode）为单位的
2. 文本文件
3. 必须指定编码格式，如 encoding='utf-8'

b 模式：二进制 /bytes 模式，除文本文件外的其他文件，如图片，视频等等       
...
这里使用了文件的 .read() 方法来读出其中的内容，并存放在变量 res 中  
然后就可以打印出来知道其中的内容了   
...
解决跨平台路径问题，推荐使用 os 模块    
该模块下的 path 函数用于读取文件/文件夹名称( str 类型)     
配合该函数的 join 方法拼接出完整路径     
这种方法要求要知道所有的文件夹名称  
```
import os
...
因为操作系统打开文件有上限，如果忘了 close 文件，到达上限之后，下面的文件就打不开了  
所以，为了防止这样的情况，Python 引入了 with 关键字来根据上下文自动打开\关闭文件   
...
with open('a.txt',mode='rt') as f1,\       # 注意这里的格式，一个反斜杠\表示换行，继续输入，用在较长的语句中
    ...
    res1=f1.read()  # t 模式会将 f.read() 读出的结果解码成 unicode
    res2=f2.read()
    print(res1)
    print(res2)
print("以上两个文件已经自动关闭")
...
为什么用 t 模式需要指定编码表？没有指定 encoding 参数操作系统会使用自己默认的编码     
linux 系统默认 utf-8      
windows 系统默认 gbk       
尤其是在 windows 系统下编写代码，养成加上 encoding 的好习惯 
...
as 表示把这个文件读出之后放在一个叫 f 的文件句柄内，相当于 `f = open()` 
read 方法在读取大文件时容易填满内存    
在调用 read 方法后，*文件指针*移动，并读出文件内容直到文件结尾     
此后，再次调用 read 方法将无法读出文件内容，也就是说文件只可以被读取一次，直到被关闭后再次读取  
...
# 这里为什么要用 .strip() 方法？  
# 因为你在输入的时候，把回车键的符号(\n)也输入了
# 例如输入 lilei，实际上你输入的是 lilei\n

# 验证
with open('user.txt',mode='rt',encoding='utf-8') as f:
    for line in f:
        # print(line,end='')  # 用这行代码可以发现是一行一行进行读取的
        # strip()去除空白字符，包括\n\t等等
        username,password=line.strip().split(':')
        if inp_username == username and inp_password == password:
            print('login successfull')
            break
            # 读取一行，拆分，然后再对比，如此循环直到文件最后
...
# 输入源文件路径：E:\learning_note\a.txt
# 输入目标文件路径：E:\learning_note\b.txt
# 然后就发现该文件夹下多了一个 b.txt
```

**a 模式**        
只追加写，在文件不存在时会创建空文档，在文件存在时文件指针会直接跳到末尾，不会清空文件，此时文件不可读，即不可使用 read 方法，用了就报错
