...
常用快捷键：  
1. ctrl+shift+p:打开VsCode命令窗口。在这个窗口下输入插件名称就能知道这个插件支持哪些特性了，顺带还会说明特性快捷键。
2. ctrl+\：新建一个编辑窗口(在当前窗口的右侧)
3. ctrl+shift+n：新建一个编辑器窗口
4. ctrl+ +/-：放大字体/缩小字体
5. ctrl+F2：多光标编辑
6. alt+鼠标左键点击：设置多光标编辑点
7. ctrl+[：移动代码块(向左移动)   
8. ctrl+[：移动代码块(向右移动)   
9. ctrl+p：文件查找，快速打开文件列表，输入关键字匹配文件，优先显示最新打开过的文件，方便的在指定文件之间跳转。
10. F12:跳转到定义，跳转到函数或符号的定义。
11. alt+F12:以预览方式在当前页面显示定义，都是查看定义，相对F12的优点是不会跳出当前文件到定义文件，而是在当前文件打开一个小窗口预览
12. shift+F12:查看光标所在函数或变量的引用，就像 Alt +F12 一样以预览方式在当前文件打开引用的文件列表。
13. alt+左/右箭头:前进或者后退到光标所在源码的上一个位置。
14. ctrl+shift+o:查看当前文件的符号，可以用关键字过滤符号，当然你也可以在左侧的大纲视图中查找符号，不过大纲视图不能查找匹配符号，所以我更习惯用快捷键方式查找符号。
...
2. float型
3. str型
4. 列表：有序，计数从0开始而不是1；可列表嵌套使用
5. 字典：key值位置，且key值必须为不可变类型，即不可使用容器类型
6. 元组：不可变的列表
7. 集合：无序，元素不重复的列表
8. 布尔值：只有两个值，True和Falsfe，主要用于条件判断
...
while/for + else：针对break      
else包含的代码会在while/for循环结束后，并且while/for循环是在没有被break打断的情况下正常结束的，才会运行        
...
基本类型的方法：python针对基本类型，内置的一些功能，可以更方便的达到一些目的    
...
swapcase,title方法
    ...
    print("hello ada lovelace".title())  # 按照标题样式，首字母大写
...
**列表的其他常用操作**    
1. find/index/count用法同str类型
注意find不报错，index报错即可
2. clear方法：清空列表
3. reverse方法：倒序排列列表
4. sort方法：排序，要求列表内的元素必须相同
